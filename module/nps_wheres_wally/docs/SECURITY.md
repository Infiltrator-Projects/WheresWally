...
Spreadsheet applications may interpret cells beginning with `=`, `+`, `-` or `@` as formulas. Current NPS fields are normally identifiers rather than arbitrary user-controlled spreadsheet expressions; nevertheless, exported data should be treated as administrative data and opened under normal organisational controls.
...
The automatic installer sets directories to mode 755 and files to mode 644, owned by root. No module file is made writable by the web-server account.
