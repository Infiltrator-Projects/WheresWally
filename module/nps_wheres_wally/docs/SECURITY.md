...
Spreadsheet applications may interpret cells beginning with formula markers such as `=`, `+`, `-` or `@`. Version 1.1.4 neutralises those prefixes with a leading apostrophe before CSV quoting so event data is opened as text rather than evaluated as a spreadsheet formula.
